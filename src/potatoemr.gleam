        ...
        h.ul([a.class("flex space-x-4")], [
          ...
          ..view_header_links(
            [
              ...
              #(Settings, "Settings"),
              #(
                RegisterPatient(form.new(patient_schema())),
    ...
    case model.route {
      RouteNoId(route) ->
        h.main([a.class("my-16 flex-1")], case route {
          ...
          RegisterPatient(newpatient) -> view_registerpatient(newpatient)
...
fn view_header_links(
  current current,
  to targets_and_displays: List(#(RouteNoId, String)),
) {
  list.map(targets_and_displays, fn(link) {
    view_header_link(to: RouteNoId(link.0), current:, label: link.1)
  })
}
...
fn view_registerpatient(newpatient: Form(r4us.Patient)) {
  [
    h.form(
      [
